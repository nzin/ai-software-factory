...
// NewServer creates a new api catalog server but does not configure it
func NewServer(api *operations.CatalogAPI) *Server {
	s := new(Server)
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/catalog.sock"`
...
func (s *Server) SetAPI(api *operations.CatalogAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving catalog at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := domainSocket.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving catalog at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving catalog at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := httpServer.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving catalog at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving catalog at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := httpsServer.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving catalog at https://%s", l.Addr())
