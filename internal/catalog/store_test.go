...
func TestUpsertAndGet(t *testing.T) {
	...
	_, mc, err := s.Upsert(ctx, a, modelext.Config{Model: "claude-opus-5"}, false)
	if err != nil {
...
func TestUpsertDoesNotOverwriteModelUnlessForced(t *testing.T) {
	...
	if _, _, err := s.Upsert(ctx, a, modelext.Config{Model: "claude-opus-5"}, false); err != nil {
		t.Fatal(err)
	}
	// Operator override.
	if _, err := s.PatchModel(ctx, "planner", ModelPatch{Model: ptr("claude-sonnet-5")}); err != nil {
		t.Fatal(err)
	}
	...
	// With force -> replaced.
	if _, _, err := s.Upsert(ctx, a, modelext.Config{Model: "claude-opus-5"}, true); err != nil {
		t.Fatal(err)
	}
	mc, _ = s.GetModel(ctx, "planner")
	if mc.Model != "claude-opus-5" {
